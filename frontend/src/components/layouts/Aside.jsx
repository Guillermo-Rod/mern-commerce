...
import { toggle } from '../../redux/sidebarAdminSlice.mjs'
import classNames from "classnames";

function Aside () {
    const collapsed = useSelector(state => state.sidebarAdmin.collapsed);

    const classes = classNames('fixed z-20 h-full top-0 left-0 pt-16 flex lg:flex flex-shrink-0 flex-col w-64 transition-width duration-75',{
        hidden: collapsed
    });

    return (
        <aside id="sidebar" className={classes} aria-label="Sidebar">
                ...
                <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                    <div className="flex-1 px-3 bg-white divide-y space-y-1">
                        <ul className="space-y-2 pb-2">
                            ...
                            {/*
                            <x-tailwind.sidebar.option link="{{ route('admin.users.index') }}" icon="fa-solid fa-user-group" text="Usuarios" />
                            <x-tailwind.sidebar.option link="{{ route('admin.testing.index') }}" icon="fa-solid fa-hand-holding-heart" text="Pruebas"/>
                            */}
                        </ul>
